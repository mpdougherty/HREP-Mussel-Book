...
The purpose of this step is to various metrics from unique sample data for input to the Maxent model.
...
## MCAT Metrics
*Each metric will serve as an individual model run in Maxent.  These metrics are based on the Mussel Community Assessment Tool (MCAT) outlined in Dunn et al. (2016) and have been modified to fit a site-specific scale (i.e., Steamboat Island). The following MCAT metrics were selected for inclusion to the model:
...
*Add the following fields (Type=long integer) to the attribute table: 'Listed'; 'Tolerant'; 'Lampsilini'; 'Juveniles';'Over_15yrs'
...
*Open the `PercentListed` attribute table.
  *Add a field titled 'Perc_listed' Type=double and enter the expression `([SUM_Listed] / [SUM_NumberLive]) * 100` into the field calculator to calculate % of individual listed species per quadrat sample.
...
## Create Percent Tolerant
  *Repeat the steps percent listed (except % tolerant, where thresholds are set at <50%) and export to .csv
...
The purpose of this step is to calculate the Pielou's evenness index (range 0-1), estimated at the species level. 
...
```{r}
# Merge the Pielou scores back on the sp object
...
* Create a table of SampleID by Pielou evenness score. Use the dissolve tool:
  * Input Features: `sample_species_pielou`
  * Output feature class: `sample_pielou`
...
* Create a new variable for Tribe. 
