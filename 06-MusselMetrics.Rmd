...
* Create a feature class respresenting all individual mussel samples in the study area and name it `Cordova_all_individuals`. 
* Add the following fields (Type=long integer) to the attribute table: `Listed`, `Tolerant`, `Lampsilini`, `Juveniles`, `Over_15yrs`, `tribe` 

## Create Samples
The purpose of this step is to create a point feature class to represent the unique sample locations (sites) where mussels have been sampled. 

* Dissolve the `Cordova_all_individuals` feature class by `SAMPLE_ID`. Add `SUM_NumberLive` in the statistics section of the tool. Uncheck create multipart features. Set Output Feature Class to the working .gdb and name it `Samples`.
